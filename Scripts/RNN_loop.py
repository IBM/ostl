...
def dynamic_rnn(cell, inputs, target_outputs, loss_function, gradient_function, train, store_LE_signal, use_BP, update_gradient_every, optimizer, global_grad=-1., 
                sequence_length=None, initial_state=None, dtype=None, parallel_iterations=None, swap_memory=False, time_major=False, scope=None):
...
def _dynamic_rnn_loop(cell,
    ...
    def _time_step(time, output_ta_t, state):
        ...
        #loss = loss_function(target_output_t, output[0])
        #grads_vars_list = gradient_function(loss)
        #grads, vars = zip(*grads_vars_list)
        #c = cum_loss + grads[1]
        #c = cum_loss + loss
        
        def update_BPTT_gradients():
            op_list = []
            
            if in_graph_mode:
                final_outputs = tuple(ta.stack() for ta in output_ta)
                # Restore some shape information
            ...
            with tf.control_dependencies(op_list):
                ...
                '''TODO: One has to tune these loss function here'''
                loss = loss_function(target_outputs, final_outputs, time)
                loss = reduce_mean(loss, (1))
                for cell_ind, c in enumerate(cell._cells):
                    #Problem that one cannot get the proper gradients within a single loop iteration from all the previous iterations.
                    #This can only be done outside after the loop is placed in the graph
                    grads = tf.gradients(loss, c.weights)
                    grads_corr = []
                    for g in grads:
                        if g == None:
                            continue
                    ...
                    op_list.extend(cell.store_bptt_grad(grads_corr, cell_ind, time))
        ...
        def update_trainable_cells_eligibility_traces(apply, time):
            ...
            def apply_wo_scaling():
                ...
                for cell_ind, c in enumerate(cell._cells):
                    '''
                    if cell_ind == 0:
                        grads = gradient_function(loss, new_outputs[cell_ind])
                        #with control_dependencies([tf.Print(grads, [grads])]):
                        call_cell_grad = lambda: cell.grad_c(grads, new_state, cell_ind, apply)
                        op_list.extend(call_cell_grad())
                        #grads = gradient_function(loss, new_outputs[0])
                        #call_cell_grad = lambda: cell.grad_c(grads, new_state, cell_ind, apply)
                    else:
                        grads = gradient_function(loss, new_outputs[cell_ind])
                        call_cell_grad = lambda: cell.grad_c(grads, new_state, cell_ind, apply)
        ...
        '''
        op_grad = tf.cond(tf.equal(train, tf.constant(1, tf.int32)), 
                          lambda: tf.cond(tf.equal(tf.constant(use_BP, tf.int32), tf.constant(1, tf.int32)), update_trainable_cells_BP, update_trainable_cells_eligibility_traces), 
                          lambda: tf.no_op())
    
        op = tf.cond(tf.logical_and(tf.equal(train, tf.constant(1, tf.int32)), tf.logical_or(\
                                                                                       tf.logical_and(tf.equal(update_gradient_every, -1), tf.equal(time, loop_bound - 1)),
        ...
        '''
        #Decision STN BPN UI
        op_sty_bpn_ui = tf.cond(tf.logical_or(\
                                   tf.logical_and(tf.equal(update_gradient_every, -1), tf.equal(time, loop_bound - 1)),
                                   tf.logical_and(tf.not_equal(update_gradient_every, -1), tf.logical_or(tf.equal(tf.mod(time, update_gradient_every), tf.constant(0, tf.int32)), tf.equal(time, loop_bound - 1)))),
                     lambda: update_trainable_cells_eligibility_traces(True), 
                     lambda: update_trainable_cells_eligibility_traces(False))
        
        #Decision STN BPY UI
        op_sty_bpy_ui = tf.cond(tf.logical_or(\
                                   tf.logical_and(tf.equal(update_gradient_every, -1), tf.equal(time, loop_bound - 1)),
                                   tf.logical_and(tf.not_equal(update_gradient_every, -1), tf.logical_or(tf.equal(tf.mod(time, update_gradient_every), tf.constant(0, tf.int32)), tf.equal(time, loop_bound - 1)))),
                     lambda: update_trainable_cells_BP(True), 
                     lambda: update_trainable_cells_BP(False))
        
        #Decision STN BP
        op_sty_bp = tf.cond(tf.equal(tf.constant(use_BP, tf.int32), tf.constant(1, tf.int32)),
                            lambda: op_sty_bpy_ui,
                            lambda: op_sty_bpn_ui)
        
        #Decision STY BPN UI
        op_stn_bpn_ui = tf.cond(tf.logical_or(\
                                   tf.logical_and(tf.equal(update_gradient_every, -1), tf.equal(time, loop_bound - 1)),
                                   tf.logical_and(tf.not_equal(update_gradient_every, -1), tf.logical_or(tf.equal(tf.mod(time, update_gradient_every), tf.constant(0, tf.int32)), tf.equal(time, loop_bound - 1)))),
                     lambda: update_trainable_cells_eligibility_traces(True), 
                     lambda: tf.no_op())
        ...
        #Decision STY BPY UI
        op_stn_bpy_ui = tf.cond(tf.logical_or(\
                                   tf.logical_and(tf.equal(update_gradient_every, -1), tf.equal(time, loop_bound - 1)),
                                   tf.logical_and(tf.not_equal(update_gradient_every, -1), tf.logical_or(tf.equal(tf.mod(time, update_gradient_every), tf.constant(0, tf.int32)), tf.equal(time, loop_bound - 1)))),
                     lambda: update_trainable_cells_BP(True), 
                     lambda: tf.no_op())
        ...
        #Decision STY BP
        op_stn_bp = tf.cond(tf.equal(tf.constant(use_BP, tf.int32), tf.constant(1, tf.int32)),
                            lambda: op_stn_bpy_ui,
                            lambda: op_stn_bpn_ui)
        
        
        #Decision ST
        op_st = tf.cond(tf.equal(tf.constant(store_LE_signal, tf.int32), tf.constant(1, tf.int32)),
                           lambda: op_sty_bp,
                           lambda: op_stn_bp)
        
        #Decision train
        op = tf.cond(tf.equal(train, tf.constant(1, tf.int32)),
                     lambda: op_st,
                     lambda: tf.no_op())
        '''

        '''Compute the ideal BPTT weight changes'''
        '''
        op = tf.cond(tf.equal(time, loop_bound - 1),
                     lambda: update_BPTT_gradients(),
                     lambda: tf.no_op())
        '''
        op = tf.no_op()
        ...
        with tf.control_dependencies([op]):
            ...
            '''
            update_cnt = tf.cond(tf.logical_and(tf.equal(train, tf.constant(1, tf.int32)), tf.logical_or(\
                                                                                           tf.logical_and(tf.equal(update_gradient_every, -1), tf.equal(time, loop_bound - 1)),
                                                                                           tf.logical_and(tf.not_equal(update_gradient_every, -1), tf.logical_or(tf.equal(tf.mod(time, update_gradient_every), tf.constant(0, tf.int32)), tf.equal(time, loop_bound - 1))))),
                                 lambda: update_cnt + 1,
                                 lambda: update_cnt)
            ...
            with tf.control_dependencies([op]):
                '''
                #Discard the gradients after each timestep
                output = [tf.stop_gradient(o) for o in output]
                new_state_wog = tuple()
                for tup in new_state:
                    typ = type(tup)
                    new_state_wog += (typ(*[tf.stop_gradient(g) for g in list(tup)]),)
                new_state = new_state_wog
                '''
                ...
                # return (time + 1, output_ta_t, new_state, update_cnt)
                # return (time + 1, output_ta_t, new_state, c)
